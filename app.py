...
def play_voice(text: str, voice_name: str):
    """Generate & play audio from ElevenLabs using the updated SDK."""
    try:
        # Fetch all voices
        voices_response = client.voices.get_all()  # returns GetVoicesResponse object

        # voices_response.voices is the actual list of voices
        voice_obj = next((v for v in voices_response.voices if v.name.lower() == voice_name.lower()), None)
        if not voice_obj:
        ...
        audio = client.text_to_speech.convert(
            voice_id=voice_obj.voice_id,  # <-- use voice_id property
...
if st.session_state["step"] == "demographics":
    ...
    st.session_state["used_assistants"] = st.radio("Q5. Have you used voice assistants before?", ["Yes", "No"])
    st.session_state["tech_comfort"] = st.radio(
        "Q6. Comfort with technology", ["Not at all", "Slightly", "Moderately", "Very", "Extremely"])

    if st.button("Continue to Baseline"):
...
if st.session_state["step"] == "baseline":
    ...
    section_header("Anxiety – GAD-7")
    gad_items = [
        "Feeling nervous, anxious, or on edge.",
        "Cannot stop/control worrying.",
    ...
    st.session_state["gad_impact"] = st.radio("Impact on life?", ["Not difficult", "Somewhat", "Very", "Extremely"])

    section_header("Current Mood – PANAS (Short Form)")
    panas_items = ["Interested","Distressed","Excited","Upset","Strong","Guilty","Scared","Hostile","Enthusiastic","Proud"]
    five_scale = [1,2,3,4,5]
    for i, label in enumerate(panas_items,start=1):
        st.session_state["panas"][f"q{i}"] = st.radio(label, five_scale, horizontal=True)
    st.session_state["single_mood"] = st.radio("Overall mood (1=very negative, 5=very positive):", [1,2,3,4,5], horizontal=True)

    if st.button("Continue to Empathetic Voice Session"):
...
if st.session_state["step"] == "session_emp":
    ...
    emp_script = st.text_area("Empathetic script:", "You're doing your best. Let's take a slow breath together...")
    if st.button("▶ Play Empathetic Voice"):
        play_voice(emp_script, "Rachel")
    st.write("Rate the voice (1=Strongly disagree, 5=Strongly agree)")
    five_scale = [1, 2, 3, 4, 5] 
    for i in range(1,9):
        st.session_state["emp"][f"q{i}"] = st.radio(f"Q{i}", five_scale, horizontal=True)
    st.session_state["emp_state_anxiety"] = st.radio("During-interaction anxiety", five_scale, horizontal=True)
    if st.button("Continue to Neutral Voice Session"):
...
if st.session_state["step"] == "session_neu":
    ...
    neu_script = st.text_area("Neutral script:", "Please proceed with the task. This is standard informational message.")
    if st.button("▶ Play Neutral Voice"):
        play_voice(neu_script, "Antoni")
    st.write("Rate the voice (1=Strongly disagree, 5=Strongly agree)")
    five_scale = [1, 2, 3, 4, 5] 
    for i in range(1,9):
        st.session_state["neu"][f"q{i}"] = st.radio(f"Q{i}", five_scale, horizontal=True)
    st.session_state["neu_state_anxiety"] = st.radio("During-interaction anxiety", five_scale, horizontal=True)
    if st.button("Continue to Open-Ended Feedback"):
...
if st.session_state["step"] == "open":
    ...
    st.session_state["open_emp"] = st.text_area("Empathetic Voice Experience:")
    st.session_state["open_neu"] = st.text_area("Neutral Voice Experience:")
    st.session_state["open_compare"] = st.text_area("Comparison of voices:")
    st.session_state["open_pref"] = st.text_area("Voice Preference:")
    st.session_state["open_empathy"] = st.text_area("Perceived Empathy:")
    st.session_state["open_trust"] = st.text_area("Trust & Usefulness:")
    st.session_state["open_triggers"] = st.text_area("Triggers & Discomfort:")
    st.session_state["open_improve"] = st.text_area("Improvement Suggestions:")
    st.session_state["open_more_1"] = st.text_area("Additional Reflections:")
    st.session_state["open_more_2"] = st.text_area("Other Thoughts:")
...
if st.session_state["step"] == "review":
    ...
    st.write("Click **Submit** to upload your responses to the Hugging Face dataset.")
    if st.button("Submit"):
        record = {
...
if st.session_state["step"] == "review":
    ...
    if st.button("Submit"):
        ...
        try:
            ...
            st.error(f"Upload failed: {e}")
>>>>>>> 25bd8cb67187be28ea49cc0f302999e0b4ef0605
